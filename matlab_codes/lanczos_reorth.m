function [lam, res, X, V, T] = lanczos_reorth(A, x0, m)
% LANCZOS  Lanczos iteration for symmetric matrices and Ritz analysis with full reorthogonalitation.
%
%   [lam, res, X, V, T] = lanczos_reorth(A, x0, m)
%
...
%     where y_i is the i-th eigenvector of T.
    ...
    for j = 2:m
        ...
        % ---Full re-orthogonalization step ---
        for k = 1:j
            mu = V(:,k)' * w;
            w = w - mu * V(:,k);
        end
